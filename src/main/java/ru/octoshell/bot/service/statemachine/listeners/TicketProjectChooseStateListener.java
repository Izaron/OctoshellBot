...
@Slf4j
@Service
public class TicketProjectChooseStateListener implements StateListener {
    ...
    public TicketProjectChooseStateListener(LocaleService localeService,
    ...
    public void drawState(UserState userState, OctoshellTelegramBot bot, Message latestMessage) {
        ...
        try {
            JSONObject jsonObject = remoteCommandsService.sendCommandWithAuth(userId, ImmutableMap.of("method", "user_projects"));
            String status = jsonObject.getString("status");
            if (StringUtils.equals(status, "fail")) {
                sendMessage.setText(localeService.getProperty(locale, "main.fail-auth"));
            } else {
                sendMessage.setText(localeService.getProperty(locale, "main.tickets.projects.message"));
